...
class Program:
    def __init__(self):
        ...
        self.token_sign_key = b64decode(self.config['DEFAULT']['SYMMETRIC_KEY'])
    ...
    def configure_play_ready_license_template(self):
        """Configures PlayReady license template.
        :return: ContentKeyPolicyPlayReadyConfiguration
        :rtype ~azure.mgmt.media.models.ContentKeyPolicyPlayReadyConfiguration
        """

        obj_content_key_policy_play_ready_license = ContentKeyPolicyPlayReadyLicense(
            ...
            content_type = ContentKeyPolicyPlayReadyContentType.ultra_violet_streaming,
            ...
            play_right = ContentKeyPolicyPlayReadyPlayRight(
                ...
                explicit_analog_television_output_restriction = ContentKeyPolicyPlayReadyExplicitAnalogTelevisionRestriction(best_effort = True, configuration_data = 2),
                allow_passing_video_content_to_unknown_output = ContentKeyPolicyPlayReadyUnknownOutputPassingOption.allowed
            )
        )

        obj_content_key_policy_play_ready_configuration = ContentKeyPolicyPlayReadyConfiguration(licenses = [obj_content_key_policy_play_ready_license])

        return obj_content_key_policy_play_ready_configuration

    def configure_widevine_license_template(self):
        """Configures widevine license template.
        :return: ContentKeyPolicyWidevineConfiguration
        :rtype ~azure.mgmt.media.models.ContentKeyPolicyWidevineConfiguration
        """

        template = '{"AllowedTrackTypes":"SD_HD","ContentKeySpecs":[{"TrackType":"SD","SecurityLevel":1,"RequiredOutputProtection":{"HDCP":"HDCP_NONE"}}],"PolicyOverrides":{"CanPlay":true,"CanPersist":true,"CanRenew":false,"RentalDurationSeconds":2592000,"PlaybackDurationSeconds":10800,"LicenseDurationSeconds":604800}}'

        obj_content_key_policy_widevine_configuration = ContentKeyPolicyWidevineConfiguration(widevine_template = template)

        return obj_content_key_policy_widevine_configuration
    ...
    def get_or_create_content_key_policy(self, resource_group_name, account_name, content_key_policy_name):
        ...
        if policy is None:
            ...
            play_ready_config = self.configure_play_ready_license_template()

            widevine_config = self.configure_widevine_license_template()

            options = []

            options.append(ContentKeyPolicyOption(configuration = play_ready_config, restriction = restriction))
            options.append(ContentKeyPolicyOption(configuration = widevine_config, restriction = restriction))

            policy = self.client.content_key_policies.create_or_update(resource_group_name, account_name, content_key_policy_name, options)
        else:
                # Get the signing key from the existing policy.
                policy_properties = self.client.content_key_policies.get_policy_properties_with_secrets(resource_group_name, account_name, content_key_policy_name)
                restriction = policy_properties.options[0].restriction
                if restriction is not None:

                    # #####################################################################
                    # There seems to be a bug of deseialize the bytearry here
                    # the <byte array, len() =??> is retured instead of an actual byte array
                    #######################################################################

                    signing_key = restriction.primary_verification_key
                    if signing_key is not None:
                        self.token_sign_key = signing_key.key_value
    ...
    def run(self):
        ...
        if job.state == JobState.finished:
            ...
            key_identifier = [key for key in locator.content_keys if key.type == StreamingLocatorContentKeyType.common_encryption_cenc][0].id

            print('Key_identifier = {}'.format(key_identifier))
            ...
            print('You can use Edge/IE11 for PlayReady and Chrome/Firefox Widevine. ')
            print()
            print('https://ampdemo.azureedge.net/?url={}&playready=true&widevine=true&token=Bearer%3D{}'.format(dash_path, token))
            print()
