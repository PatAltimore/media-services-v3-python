...
class Program:
    ...
    def create_streaming_locator(self, resource_group_name, account_name, asset_name, locator_name):
        ...
        locator = StreamingLocator(asset_name = asset_name, streaming_policy_name = 'Predefined_ClearStreamingOnly')
    ...
    def get_streaming_urls(self, resource_group_name, account_name, locator_name):
        ...
        for path in paths.streaming_paths:
            ...
            streaming_urls.append(parse.urlunparse((scheme, host, path, None, None, None)))
    ...
    def run(self):
        ...
        if job.state == JobState.finished:
            ...
            locator = self.create_streaming_locator(self.resource_group_name, self.account_name, output_asset.name, locator_name)

            urls = self.get_streaming_urls(self.resource_group_name, self.account_name, locator.name)
            for url in urls:
                print(url)

            print('Done. Copy and paste the Streaming URL into the Azure Media Player at "http://aka.ms/azuremediaplayer".')
